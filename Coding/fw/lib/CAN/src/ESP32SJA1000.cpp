...
/**
*   Sets the Filter Mode for the SJA1000 CAN Controller of the ESP32.
*   @param mode     Filter Mode. 0 for Dual Filter, 1 for Single Filter
*   @return bool success
*/
bool ESP32SJA1000Class::setFilterMode(const uint8_t &mode)
{
    bool success = true;
    ...
    {
    case 0:
        modifyRegister(REG_MOD, 0x18, 0x00); /**< Change Filter Mode */
        break;

    case 1:
        modifyRegister(REG_MOD, 0x18, 0x01); /**< Change Filter Mode */
        break;
...
/**
*   Sets the Acceptance Register of the SJA1000 CAN Controller of the ESP32.
*   @param acn      Array containing the Registers to Write
*   @return bool success
*/
bool ESP32SJA1000Class::setACRn(const uint8_t *acn)
{
    bool success = true;

    modifyRegister(REG_MOD, 0x17, 0x01); // reset

    writeRegister(REG_ACRn(0), acn[0]);
    writeRegister(REG_ACRn(1), acn[1]);
    writeRegister(REG_ACRn(2), acn[2]);
    writeRegister(REG_ACRn(3), acn[3]);
...
/**
*   Sets the Acceptance Mask Register of the SJA1000 CAN Controller of the ESP32
*   @param amn     Array containing the Registers to Write
*   @return bool success
*/
bool ESP32SJA1000Class::setAMRn(const uint8_t *amn)
{
    bool success = true;

    modifyRegister(REG_MOD, 0x17, 0x01); // reset

    writeRegister(REG_AMRn(0), amn[0]);
    writeRegister(REG_AMRn(1), amn[1]);
    writeRegister(REG_AMRn(2), amn[2]);
    writeRegister(REG_AMRn(3), amn[3]);
